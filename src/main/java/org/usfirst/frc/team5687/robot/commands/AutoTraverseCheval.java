...
public class AutoTraverseCheval extends CommandGroup {
    public AutoTraverseCheval() {
        addSequential(new RunArmsManually(.5,1,1));//TODO: add in set time and speed to raise arms.    Raise arms by running motor at a set time and speed to a desired distance.
        addSequential(new AutoDrive(.5, true));//Drive forward and stop moving when at correct angle
        addSequential(new RunArmsManually(.5, 1, -1));//TODO: add in set time and speed to lower arms. While stopped, lower arms.
        addSequential(new AutoDrive(.5, 16.0));//TODO: add in double distance to drive until the center point of the cheval
        addSequential(new RunArmsManually(.5, 1, 1));//Is same as beginning
